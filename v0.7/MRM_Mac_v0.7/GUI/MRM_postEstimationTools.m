...
% Check missing data
if sum(voxData(:) == 0) > 0 || sum(isnan(voxData(:))) > 0
   msgbox('There is missing data at this voxel, cannot draw the plots')
   return
end
...
if sum(voxData(:) == 0) > 0 || sum(isnan(voxData(:))) > 0
   msgbox('There is missing data at this voxel, only the raw data has been saved')
   return
end
...
for i = 1:size(MRM.Design.X.X,2)
    for j = 1:nDVs
        tempPE(i,j) = spm_data_read(spm_data_hdr_read([out filesep 'MRM_PE_' num2str(i) '_' num2str(j) '.nii']), ...
                                    'xyz',                                                                       ...
                                    [matCoords(1) matCoords(2) matCoords(3)]');
    end
end
...
if sum(isnan(tempPE(:))) > 0
    msgbox('Parameter estimates are empty at this voxel')
    return
end
...
% Check for missing data
if sum(isnan(tempPE(:))) > 0
    msgbox('Parameter estimates are empty at this voxel. Nothing will be saved.')
    return
end
...
% Check for missing data
if sum(voxData(:) == 0) > 0 || sum(isnan(voxData(:))) > 0
    msgbox('There is missing data at this voxel. Cannot compute the tests.')
    return
end
...
% Check missing data
if sum(tempVCOV(:) == 0) > 0 || sum(isnan(tempVCOV(:))) > 0
    msgbox('There is missing data at this voxel. Cannot draw the covariance structure.')
    return
end
